...
public class Chaining {
    ...
    public int chain (int key) {
    	...
    	int hashVal = (this.A * key) % power2(this.w) >> (this.w-this.r);
        return hashVal;
    }
    ...
    public int insertKey(int key){
    	...
    	ArrayList<Integer> bin = this.Table.get(hashVal);
    	if (bin.isEmpty()) {
    		bin.add(0, key);
    		return 0;
    	}else {
    		int collision = bin.size();
    		bin.add(collision, key); // add key at the end of the bin
    		return collision;
    	}
